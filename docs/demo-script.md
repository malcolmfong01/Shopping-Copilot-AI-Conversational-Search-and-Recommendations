...
| Time | On screen | Say |
|------|-----------|-----|
| 0–12s | Header: `Hit Rate 97% · 0.853 · 8× baseline`. Empty chat. | Shopping search usually takes many turns of “what color, what size.” Ours finds the target in about 2.7 turns. |
| 12–25s | Same header; cursor over the score badge. | This is retrieval and constraint ranking — not a chatbot wrapper. Composite 0.853, eight times the baseline. |
...
- If Groq/Gemini is slow or unset, film with **LLM rank · fallback** visible and say so in the 50–75s beat.
